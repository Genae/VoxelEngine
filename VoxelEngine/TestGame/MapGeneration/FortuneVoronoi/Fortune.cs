...
using System.Linq;
using OpenTK;
using TestGame.MapGeneration.FortuneVoronoi.Events;
using TestGame.MapGeneration.FortuneVoronoi.Nodes;
using TestGame.MapGeneration.FortuneVoronoi.VoroniGraph;
using TestGame.MapGeneration.Graph;

namespace TestGame.MapGeneration.FortuneVoronoi
{
    public abstract class Fortune
	{
		...
		public static VoronoiGraph FilterVg(VoronoiGraph vg, double minLeftRightDist)
		{
			...
			foreach(var ve in vg.GetEdges().Select(edge => edge as VoronoiEdge))
			{
				if((ve.LeftData-ve.RightData).Length>=minLeftRightDist)
					vgErg.AddEdge(ve);
			}
